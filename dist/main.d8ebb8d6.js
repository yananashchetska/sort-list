...
var formatSalary = function formatSalary(salary) {
  var formatted = Number(salary.replace(/\D/g, ''));
  return formatted;
};
var makeEmployesList = function makeEmployesList(list) {
  return list.map(function (listElement, index) {
    return {
...
var sortList = function sortList(list) {
  return list.sort(function (elementA, elementB) {
    return formatSalary(elementB.salary) - formatSalary(elementA.salary);
  });
};
var liList = Array.from(document.getElementsByTagName('li'));
console.log('li list: ', liList);
var employees = makeEmployesList(liList);
var unsortedEmpl = _toConsumableArray(employees);
